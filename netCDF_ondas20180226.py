...
dfonda = pd.DataFrame(data = wave[:,yy,xx], index=datahora)
dfperiodo = pd.DataFrame(data = waveT[:,yy,xx], index=datahora)
dfdir = pd.DataFrame(data = wavedir[:,yy,xx], index=datahora)


dfperiodo.groupby(dfperiodo.index.year).std()
dirando = dfdir[dfdir.index.year == 2007].std()
ondaano = dfonda[dfonda.index.year == 2007].std()

a = dfonda.groupby(pd.Grouper(freq='2QS')).mean()
a = dfonda.groupby(pd.Grouper(freq='2QS')).std()

a = dfperiodo.groupby(pd.Grouper(freq='2QS')).mean()
a = dfperiodo.groupby(pd.Grouper(freq='2QS')).std()

a = dfdir.groupby(pd.Grouper(freq='2QS')).mean()
a = dfdir.groupby(pd.Grouper(freq='2QS')).std()
...
ax1.plot(dfdir[dfdir.index.year == 2008], 'oc')
ax1.set_xticks([])
#ax1.set_xlim(0, 1464)
...
ax2.plot(dfperiodo[dfperiodo.index.year == 2008], 'orange')
ax2.set_xticks([])
#ax2.set_xlim(0, 1464)
...
ax3.plot(dfonda[dfonda.index.year == 2008], 'g')
#ax3.set_xlim(0, 1464)
...
"*********************************************************************"
"                                                                     "
"                           SALVANDO SAIDE DE ONDA                    "
"                           PARA ENTRADA NO MODELO                    "
"                                                                     "
"*********************************************************************"

pathname = u'D:\GD\Mestrado\DISSERTACAO\ANALISE_DADOS\Ondas\NetCDF_ERA\Saida_ERA'
# Pegando ponto profundo para entrada no modelo
serieonda = wave[:, 4, 5]
serietp = waveT[:,4,5]
seriedir = wavedir[:,4,5]
# serieonda = serieonda[~serieonda.mask]
dfserieonda = pd.DataFrame(data = serieonda, index=datahora)
dfserietp = pd.DataFrame(data = serietp, index=datahora)
dfseriedir = pd.DataFrame(data = seriedir, index=datahora)
dfserieonda.to_csv('D:\\GD\Mestrado\\DISSERTACAO\\ANALISE_DADOS\\Ondas\\NetCDF_ERA\\Saida_ERA\\serieondahs20180409.csv', sep=';')
dfserietp.to_csv('D:\\GD\Mestrado\\DISSERTACAO\\ANALISE_DADOS\\Ondas\\NetCDF_ERA\\Saida_ERA\\serieondatp20180409.csv', sep=';')
dfseriedir.to_csv('D:\\GD\Mestrado\\DISSERTACAO\\ANALISE_DADOS\\Ondas\\NetCDF_ERA\\Saida_ERA\\serieondadir20180409.csv', sep=';')
