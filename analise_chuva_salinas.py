...
dados_inmet = dados_inmet[dados_inmet.index.year == 2016]
...
chuva_mensal_INMET = dados_inmet.groupby(dados_inmet.index.month).chuva.sum()
...
if grafico == 1:
    ...
    plt.ylim([0, 899])
    plt.savefig(pathname + '\\ingles_compara_clima_2016.png', format='png', dpi=1200)
...
chuva_periodo_chuvoso = periodo_chuvoso[periodo_chuvoso.chuva > lim_chuva]
# AGRUPA POR HORA
...
dias_chuva_chuvoso = chuva_periodo_chuvoso.chuva.groupby(
                                    by=chuva_periodo_chuvoso.index.hour,
...
media_chuva_verao = periodo_chuvoso.groupby(
        by=periodo_chuvoso.index.hour).chuva.mean()


"###########################################################################"
# MESMA COISA PARA O PERIODO SECO
chuva_periodo_seco = periodo_seco[periodo_seco.chuva > lim_chuva]
# chuva_periodo_seco = chuva_periodo_seco.groupby(
#                                by=chuva_periodo_seco.index.hour,
#                                sort=True, group_keys=True)
dias_chuva_periodo_seco = chuva_periodo_seco.chuva.groupby(
                                    by=chuva_periodo_seco.index.hour,
                                    sort=True, group_keys=True).count()*100./(
                                                        len(periodo_seco)/24.)
...
chuva_periodo_transicao = periodo_transicao[periodo_transicao.chuva > lim_chuva]
# chuva_periodo_transicao = chuva_periodo_transicao.groupby(
#                                by=chuva_periodo_transicao.index.hour,
#                                sort=True, group_keys=True)
dias_chuva_transicao = chuva_periodo_transicao.chuva.groupby(
                                by=chuva_periodo_transicao.index.hour,
                                sort=True, group_keys=True).count()*100./(
...
media_trans = periodo_transicao.chuva.groupby(
