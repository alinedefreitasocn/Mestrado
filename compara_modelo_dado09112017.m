...
if pergunta == 's';
	...
	M(i).wl  = vs_let(trih, 'his-series', 'ZWL'); %wl = wl(5:end, :);  
	M(i).des = descricao;
	...
	M.time    = gregorian(julian(2011,7,1):10/(24*60):julian(2011,8,31));
...
disp(1:1:lenght(M(1).names)+1;  M(1).names);
plot(M.time, M.wl,'color',rand(1,3))
...
for i = 1:1:40
plot(M.datenum, M.wl(:,i), M.datenum, M.wl10cm(:,i), 'r--', ...
	M.datenum, M.wl50cm(:,i), 'g--');
